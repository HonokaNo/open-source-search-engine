...
void *RdbMem::allocData ( char *key , int32_t dataSize , collnum_t collnum ) {
	...
	if ( m_rdb->m_inDumpLoop ) {
		...
		//     (PTRTYPE)m_ptr1,(PTRTYPE)m_ptr2,(PTRTYPE)m_mem);
			...
			// note it
			//if ( m_ks == 16 )
			//log("rdbmem: ptr2a=%"U INT32 " size=%" INT32 "",
		...
		// note it
		//if ( m_ks == 16 )
		//log("rdbmem: ptr2b=%"U INT32 " size=%" INT32 "",
	...
	if ( m_ptr1 > m_ptr2 ) {
		...
		// note it
		//if ( m_ks == 16 )
		//log("rdbmem: ptr1a=%"U INT32 " size=%" INT32 "",(int32_t)m_ptr1,dataSize);
		// return the ptr
		return m_ptr1;
	}
	// . if it's growing up...
	...
	// note it
	//if ( m_ks == 16 )
	//log("rdbmem: ptr1b=%"U INT32 " size=%" INT32 "",(int32_t)m_ptr1-dataSize,dataSize);
	// return the ptr
...
void RdbMem::freeDumpedMem( RdbTree *tree ) {
	...
	// debug
	//logf(LOG_DEBUG,
	//     "db: freeing dumped mem ptr1=%"X INT32 " ptr2=%"X INT32 ".",m_ptr1,m_ptr2);
