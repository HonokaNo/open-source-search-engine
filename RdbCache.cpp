...
bool RdbCache::getRecord ( collnum_t collnum   ,
	...
	*rec = p;
	...
	//	logf(LOG_DEBUG,
	//	     "db: promote=%"U INT32 " "
	//	     "start1=%"U INT32 " end1=%"U INT32 " "
	//	     "start2=%"U INT32 " end2=%"U INT32 " "
	//	     "rec=%"U INT32 " m_tail=%"U INT32 " bufs[0]=%"U INT32 " total=%"U INT32 "",
...
bool RdbCache::addRecord ( collnum_t collnum ,
	...
	*(int32_t *)p = timestamp; p += 4;
	...
	// debug msg (MDW)
	// if ( this == &g_spiderLoop.m_winnerListCache ) {
	// log("cache: adding rec @ %"U INT32 " size=%i tail=%" INT32 "",
	//     i1c,(int)(p-start),m_tail);
	// log("cache: stored k.n1=%"U INT32 " k.n0=%" UINT64 " %" INT32 " bytes @ %"U INT32 " tail=%"U INT32 "",
	//     ((key_t *)cacheKey)->n1,
	//     ((key_t *)cacheKey)->n0,(int)(p-start),i1c,m_tail);
	// }
	//if ( m_cks == 4 )
	//	log("stored k=%"X INT32 " %" INT32 " bytes @ %"U INT32 "",
	...
	//log("%s addRecord %" INT32 " bytes @ offset=%" INT32 " k.n1=%"U INT32 " n0=%" UINT64 " "
	//     "TOOK %" INT64 " ms" , 
	//     m_dbname , need , i , 
...
void RdbCache::removeKey ( collnum_t collnum , char *key , char *rec ) {
	...
	// debug msg
	//log("%s removing key.n1=%"U INT32 " key.n0=%" UINT64 "",m_dbname,key.n1,key.n0);
	//if ( m_cks == 4 )
	//	log("removing k=%"X INT32 "",*(int32_t *)key);
	// chain
	while ( m_ptrs[n] && 
		( *(collnum_t *)(m_ptrs[n]+0                ) != collnum ||
	...
	//log("cache: %s removing key.n1=%"U INT32 " key.n0=%" UINT64 " from slot #%" INT32 "",
...
void RdbCache::addKey ( collnum_t collnum , char *key , char *ptr ) { 
	...
	if ( ! m_ptrs[n] ) {
		...
		// debug msg 
		//key_t *k = (key_t *)key;
		//log("cache: %s added key.n1=%"U INT32 " key.n0=%" UINT64 " to slot #%" INT32 " "
		//    "ptr=0x%"X INT32 " off=%" INT32 " size=%" INT32 "",
		//    m_dbname,k->n1,k->n0,n,ptr,ptr-m_bufs[0],
		//    *(int32_t *)(ptr+2+12+4));
	}
	// debug msg
	//else 
	//	log("%s update key.n1=%"U INT32 " key.n0=%" UINT64 " in slot #%" INT32 "",
...
bool RdbCache::load ( char *dbname ) {
	...
	for ( int32_t i = 0 ; i < m_numPtrsMax ; i++ , poff++ ) {
		...
		// debug msg
		//key_t kk = *(key_t *)p;
		//log("loaded k.n1=%"U INT32 " k.n0=%" UINT64 "",kk.n1,kk.n0);
		//if ( m_fixedDataSize || m_supportLists )
		//	log("loaded k.n1=%"U INT32 " k.n0=%" UINT64 " size=%" INT32 "",
