...
//log(LOG_WARN, "rdbbuckets last key: ""%016" XINT64 "%08"X INT32 " num keys: %" INT32 "",
...
bool RdbBuckets::selfTest(bool thorough, bool core) {
	...
	for(int32_t i = 0; i < m_numBuckets; i++) {
		...
		char* kk = b->getEndKey();
		//log(LOG_WARN, "rdbbuckets last key: ""%016" XINT64 "%08"X INT32 " "
		//"num keys: %" INT32 "",
	...
	for(int32_t i = start; 
		...
		if(startKey && KEYCMP(currKey, startKey, ks) < 0) {
			log("db: key is outside the "
			    ...
			    "  it is < startkey."
			    "  %016" XINT64 "%08"X INT32 " %016" XINT64 "%08"X INT32 "."
			    "  getting keys %" INT32 " to %" INT32 " for list"
			    "bounded by %016" XINT64 "%08"X INT32 " %016" XINT64 "%08"X INT32 "",
