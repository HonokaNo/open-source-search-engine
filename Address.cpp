...
int32_t Address::print2 ( int32_t i , SafeBuf *pbuf , int64_t uh64 ) { 
	...
	if ( pbuf ) {
		...
		//pbuf->safePrintf("<td>0x%"X INT32 "</td>", m_section->m_tagHash);
...
void gotMsg2cReplyWrapper ( void *state , void *state2 ) {
	...
	// show it
	//log("addr: got reply=%" INT32 " replyaddr=0x%"X INT32 "",(int32_t)*r,(int32_t)r);
...
bool Addresses::setFirstPlaceNums ( ) {
	...
	for ( int32_t i = 0 ; i < m_numSorted ; i++ ) {
		...
		log("dbug: (a=%" INT32 ",b=%" INT32 ") sec=%"X INT32 " %s addr=%" INT32 " alias=%" INT32 " "
		    "url=%s",
		    a,b,
		    (int32_t)sp[a],
		    tmp.getBufStart() ,
