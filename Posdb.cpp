...
bool Posdb::init ( ) {
	...
	for ( ; p < pend ; p++ )
		log("db: %02" INT32 ") 0x%02"X INT32 "",p-list.m_list,
...
void PosdbTable::evalSlidingWindow ( char **ptrs , 
	...
	for ( int32_t j = i+1 ; j < nr ; j++ ) {
		...
		//log("winner %" INT32 " x %" INT32 " 0x%"X INT32 " 0x%"X INT32 "",i,j,
...
void PosdbTable::intersectLists9_r ( ) {
	...
	if ( m_r->m_getSectionStats ) {
		...
		for ( ; p < pend ; ) {
			...
			m_dt.addKey ( &sh32 );
			// log it
			//log("usite: %08"X INT32 " %" INT64 " rs=%" INT32 "",sh32,d,rs);
	...
	*/
	...
	for ( int32_t j = 0 ; j < m_q->m_numTerms ; j++ ) {
		...
		//     ( val32 != lastVal || firstTime ) ) {
			...
			fe=(FacetEntry *)ft->getValue(&val32);
			// debug 
			//log("facets: got entry for key=%"U INT32 " "
