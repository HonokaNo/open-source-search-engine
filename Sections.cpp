...
bool SectionVotingTable::addVote3 ( int32_t        turkTagHash ,
	...
	// print out for debug
	//log("section: adding vote #%" INT32 ") th=%"U INT32 "-%" INT32 "",
...
bool SectionVotingTable::hash ( int64_t docId , 
	...
	for ( int32_t i = 0 ; i < m_svt.m_numSlots ; i++ ) {
		...
		//logf(LOG_DEBUG,"section: added tagHash=0x%"X INT32 " "
...
bool Sections::print ( SafeBuf *sbuf ,
	...
	for ( int32_t i = 0 ; i < nw ; i++ ) {
		...
		for ( ; ns>0 && dstack[ns-1]->m_b == i ; ns-- ) {
			//sbuf->safePrintf("</div id=0x%"X INT32 ">\n",
			//(int32_t)dstack[ns-1]);
			sbuf->safePrintf("</div>\n");
		}
		...
		if ( //tids[i] && 
			...
			for ( int32_t k = qq-1 ; k >= 0 ; k-- ) {
				...
				// start the new div
				//sbuf->safePrintf("<div id=0x%"X INT32 " "//e%" INT32 " "
				sbuf->safePrintf("<div "
						 "style=\""
						 "background-color:#%06"X INT32 ";"
						 "margin-left:20px;"
						 "border:#%06"X INT32 " 1px solid;"
						 "color:#%06"X INT32 "\">",
						 //(int32_t)sk,
						 bcolor,
						 rcolor,
	...
	for ( Section *sn = m_rootSection ; sn ; sn = sn->m_next ) {
		...
		if ( sn->m_contentHash ) {
			int32_t modified = sn->m_tagHash ^ sn->m_contentHash;
			sprintf(ttt,"0x%"X INT32 "",modified);
			xs = ttt;
		}
		// int16_tcut
