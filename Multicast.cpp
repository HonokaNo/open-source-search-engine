...
void Multicast::sendToGroup ( ) {
	...
	for ( int32_t i = 0 ; i < m_numHosts ; i++ ) {
		...
		//logf(LOG_DEBUG,"net: mcast state=%08"X INT32 "",(int32_t)this);
#endif
		...
		if ( us->sendRequest ( m_msg       , 
			...
			//logf(LOG_DEBUG, "net: mcast slotPtr=%08"X INT32 "",
			//     (int32_t)&m_slots[i]);
#endif
			continue;
		}
...
void Multicast::gotReply2 ( UdpSlot *slot ) {
	...
	if ( i == m_numHosts ) {
		//log("not our slot: mcast=%"U INT32 "",(int32_t)this);
...
#ifdef _GLOBALSPEC_
	// debug message for global spec
	//logf(LOG_DEBUG,"net: mcast2 state=%08"X INT32 "",(int32_t)this);
#endif
	// sanity check
...
#ifdef _GLOBALSPEC_
	// note the slot ptr for reference
	//logf(LOG_DEBUG,"net: mcast2 slotPtr=%08"X INT32 "",(int32_t)&m_slots[i]);
#endif
...
#ifdef _GLOBALSPEC_
	// debug msg
	//logf(LOG_DEBUG,"net: mcast registered1 this=%08"X INT32 "",(int32_t)this);
#endif
	// successful launch
	return true;
}
...
#ifdef _GLOBALSPEC_
		// debug msg
		//logf(LOG_DEBUG,"net: mcast unregistered1 this= %08"X INT32 "",
		//     (int32_t)this);
#endif
	}
	// unregister our sleep wrapper if we did
