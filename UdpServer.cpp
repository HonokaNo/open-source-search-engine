...
int32_t UdpServer::readSock_ass ( UdpSlot **slotPtr , int64_t now ) {
	...
	*slotPtr = NULL;
	...
	if ( h ) ip2 = h->m_ip;
	//logf(LOG_DEBUG,"net: got h=%"U INT32 "",(int32_t)h);
 ...
 discard:
	...
	*/
	...
	if ( ! isAck                             &&
		...
		log("%s token to transId=%" INT32 " msgType=0x%hhx callback=%08"X INT32 ""
		    " slot=%"U INT32 "", a,slot->m_transId , slot->m_msgType,
	...
	if (   isAck                             && 
		...
		char *a = "Gave";
		if ( *s_token || s_local ) a = "Stole";
		log("%s token to transId=%" INT32 " msgType=0x%hhx slot=%"U INT32 "",
...
bool UdpServer::makeCallback_ass ( UdpSlot *slot ) {
	...
	*/
	...
	if ( slot->m_callback ) { 
		...
		"msgType=0x%hhx g_errno=%s callback=%08"X INT32 "",
	...
	if ( slot->m_calledHandler ) {
		...
		if ( *s_token == slot || s_local == slot ) {
			...
			log("udp: makeCallback_ass: done sending "
			    "slot=%"U INT32 " bytes=%" INT32 "", (uint32_t)slot , 
			    slot->m_sendBufSize);
			log("s_token released");
		}
#endif
	...
	//if ( took > 10 )
	//	log(mt,"net: Handler transId=%" INT32 " slot=%"U INT32 " "
...
void UdpServer::timePoll ( ) {
	...
	// some debug info
	if ( *s_token ) 
		log("s_token  occupied by slot=%"U INT32 " age=%"U INT32 "",
		    (uint32_t)*s_token,
		    (uint32_t)now - *s_tokenTime );
	if (  s_local ) 
		log("s_local  occupied by slot=%"U INT32 " age=%"U INT32 "",
		    (uint32_t)s_local,
	...
	for ( UdpSlot *slot = m_head2 ; slot ; slot = slot->m_next2 ) {
		...
		if ( (slot == *s_token || slot == s_local) && elapsed >= 30 ) {
			...
			log("Token freed up (no more acks/dgrams read) for "
			    "transId=%" INT32 " msgType=0x%hhx weInitiated=%08"X INT32 "",
