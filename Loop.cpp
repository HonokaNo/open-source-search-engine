...
void Loop::unregisterCallback ( Slot **slots , int fd , void *state ,
	...
	while ( s ) {
		...
		// debug msg
		//log("Loop::unregistered fd=%" INT32 " state=%"U INT32 "", fd, (int32_t)state );
...
bool Loop::addSlot ( bool forReading , int fd, void *state, 
	...
	// debug msg
	//log("Loop::registered fd=%i state=%"U INT32 "",fd,state);
...
void Loop::callCallbacks_ass ( bool forReading , int fd , int64_t now ,
	...
	while ( s ) {
		...
		//	log("Loop::callCallbacks_ass: for fd=%" INT32 " state=%"U INT32 "",
