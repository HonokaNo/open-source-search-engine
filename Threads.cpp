...
void Threads::setPid ( ) {
	...
	//log(LOG_INFO,
	//    "threads: main process THREAD id = %"U INT32 "",(int32_t unsigned)s_pid);
...
ThreadEntry *ThreadQueue::addEntry ( int32_t   niceness                     , 
	...
	// }
	// debug msg
	//fprintf(stderr,"addEntry my pid=%"U INT32 "\n", (int32_t)getpid() );
	// get an available entry
	//ThreadEntry *t = &m_entries [ i ];
	// debug msg
	//log("claiming entry state=%"U INT32 ", occupied=%" INT32 "",(int32_t)t->m_state, 
	...
	t->m_stack        = NULL;
	// debug msg
	//log("setting t=%"U INT32 " m_isDone to 0", (int32_t)t );
	t->m_isDone       = false;
...
bool ThreadQueue::cleanUp ( ThreadEntry *tt , int32_t maxNiceness ) {
	...
	again:
		...
		*/
		...
		// debug msg
		//log("[%"U INT32 "] CLEANING UP THREAD type=%" INT32 ", numLaunched=%" INT32 "",
		//     m_entries[i].m_tid , m_threadType , m_launched );
		// remove it
		// debug msg
		//log("CLN  TID=%"U INT32 " t=%"U INT32 "",(int32_t)t->m_tid , (int32_t)t);
		//log("thread callback for tid=%"U INT32 "",(int32_t)t->m_tid );
...
// 		}
		...
		//g_errno = 0;
		// debug msg
		//log("CLN2 TID=%"U INT32 " t=%" INT32 "",(int32_t)t->m_tid ,(int32_t)t);
...
bool ThreadQueue::launchThreadForReals ( ThreadEntry **headPtr ,
	...
	*/
	// debug msg
	//if ( m_threadType == 0 ) 
	//	log("creating thread, t=%"U INT32 " state=%"U INT32 " 
	//launched = %" INT32 "", 
...
int startUp ( void *state ) {
	...
	if ( setpriority ( PRIO_PROCESS, getpidtid() , p ) < 0 ) {
		...
		if ( s_firstTime ) {
			log("thread: Call to "
			    "setpriority(%"U INT32 ",%i) in thread "
			    "failed: %s. This "
