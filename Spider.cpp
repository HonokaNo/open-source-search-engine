...
int32_t SpiderRequest::printToTable ( SafeBuf *sb , char *status ,
	...
	//sb->safePrintf(" <td>0x%"X INT32 "</td>\n",m_hostHash32 );
	//sb->safePrintf(" <td>0x%"X INT32 "</td>\n",m_domHash32 );
	//sb->safePrintf(" <td>0x%"X INT32 "</td>\n",m_siteHash32 );
...
bool SpiderLoop::gotDoledbList2 ( ) {
	...
	/*if ( g_conf.m_logDebugSpider ) {
		...
		logf(LOG_DEBUG,"spider: loading list from doledb startkey=%s"
		     " pri=%" INT32 " time=%"U INT32 " uh48=%" UINT64 "",
...
bool SpiderLoop::indexedDoc ( XmlDoc *xd ) {
	...
	if ( ! strcmp(xd->m_coll,"qatest123") && ! respider &&
		...
		char fn[1024]; 
		sprintf(fn,"%s/%s/parse.%" UINT64 ".%"U INT32 ".html",
		...
		sprintf(fn,"%s/%s/parse-int16_tdisplay.%" UINT64 ".%"U INT32 ".html",
			g_hostdb.m_dir,testDir,h,g_test.m_runId);
		...
		char cf[1024];
		sprintf (cf,"%s/%s/critical.%" UINT64 ".%"U INT32 ".txt",
			 g_hostdb.m_dir,testDir,h,g_test.m_runId);
		// save to that
		ttt.dumpToFile ( cf );
		//char cmd[256];
		//sprintf(cmd,"touch %s/test/critical.%" UINT64 ".%"U INT32 ".txt",
...
bool sendPage ( State11 *st ) {
	...
	*/
	...
	strftime ( time , 256 , "%b %e %T %Y UTC", timeStruct );
	sb.safePrintf("</b>" //  (current time = %s = %"U INT32 ") "
		      "</td></tr>\n" 
		      //,time,nowUTC
		      );
