...
bool RdbList::checkList_r ( bool removeNegRecs , bool sleepOnProblem ,
	...
	while ( ! isExhausted() ) {
		...
		if ( KEYCMP(k,acceptable,m_ks)>0 ) {
			...
			//log("db: k.n1=%"X INT32 " k.n0=%" XINT64 "",k.n1,k.n0);
			log("db: k2=%s",KEYSTR(k,m_ks));
			log("db: ak=%s",KEYSTR(acceptable,m_ks));
			//log("db:e.n1=%"X INT32 " e.n0=%" XINT64 "",m_endKey.n1,m_endKey.n0);
	...
	if ( m_lastKeyIsValid && KEYCMP(oldk,m_lastKey,m_ks) != 0 ) {
		...
		log(LOG_LOGIC,
		    //"db: rdbList: checkList_r: k.n1=%"X INT32 " k.n0=%" XINT64 "",
		    //oldk.n1,oldk.n0);
		    "db: rdbList: checkList_r: key=%s",
		    KEYSTR(oldk,m_ks));
		log(LOG_LOGIC,
		    //"db: rdbList: checkList_r: l.n1=%"X INT32 " l.n0=%" XINT64 "",
...
bool RdbList::checkIndexList_r ( bool removeNegRecs , bool sleepOnProblem ) {
	...
	if ( status < 0 ) {
		...
		//log("db: k.n1=%"X INT32 " k.n0=%" XINT64 "",
		//    k.n1,k.n0);
		//log("db: s.n1=%"X INT32 " s.n0=%" XINT64 "",
 ...
 loop:
	...
	if ( m_lastKeyIsValid && 
		...
		//log(LOG_LOGIC,"db: k.n1=%"X INT32 " k.n0=%" XINT64 "",k.n1,k.n0);
		//log(LOG_LOGIC,"db: l.n1=%"X INT32 " l.n0=%" XINT64 "",
		//    m_lastKey.n1,m_lastKey.n0);
 ...
 top:
	...
	if ( groupId != myGroupId ) { 
		if ( g_conf.m_allowScale ) {
			...
			log ( "db: Found invalid rec in db. key=%"X INT32 " %" XINT64 " "
...
bool RdbList::indexMerge_r ( RdbList **lists         ,  
	...
	if ( numLists == 0 ) return true;
#ifdef _MERGEDEBUG_
	//log(LOG_INFO,"mdw: sk.n1=%"U INT32 " sk.n0=%" UINT64 " ek.n1=%"U INT32 " ek.n0=%" UINT64 "",
	...
	// debug msg
	//log("merge start.n1=%"X INT32 " n0=%" XINT64 "", m_startKey.n1 , m_startKey.n0 );
	//log("merge end  .n1=%"X INT32 " n0=%" XINT64 "", m_endKey.n1   , m_endKey.n0   );
