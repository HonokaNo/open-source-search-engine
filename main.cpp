	...
	*/
...

	{
		for ( int32_t uc = 0 ; uc < 0xe01ef ; uc++ ) {
			...
			fprintf(stderr,"%"X INT32 ") %s --> ",uc,obuf);
			// sanity
			...
			for ( int32_t i = 0 ; i < klen ; i++ ) {
				...
				// show the #
				fprintf(stderr,"{%"X INT32 "}",(int32_t)ui);
...
int scale ( char *newHostsConf , bool useShotgunIp) {
	...
	for ( int32_t i = 0 ; i < 1000 ; i++ ) {
		...
		if ( (groupId2 & hdb1->m_groupMask) != groupId1 ) {
			log("Bad engineer. Group id 0x%"X INT32 " not derivative of "
			    "group id 0x%"X INT32 " for titledb.",groupId2,groupId1);
			return -1;
		}
		*/
		/*
		...
		if ( (groupId2 & hdb1->m_groupMask) != groupId1 ) {
			log("Bad engineer. Group id 0x%"X INT32 " not derivative of "
			    "group id 0x%"X INT32 " for checksumdb.",
			    groupId2,groupId1);
			return -1;
		}
		*/
		...
		if ( (groupId2 & hdb1->m_groupMask) != groupId1 ) {
			log("Bad engineer. Group id 0x%"X INT32 " not derivative of "
			    "group id 0x%"X INT32 " for spiderdb.",
			    groupId2,groupId1);
			return -1;
		}
		...
		if ( (groupId2 & hdb1->m_groupMask) != groupId1 ) {
			log("Bad engineer. Group id 0x%"X INT32 " not derivative of "
			    "group id 0x%"X INT32 " for clusterdb.",
			    groupId2,groupId1);
			return -1;
		}
		...
		if ( (groupId2 & hdb1->m_groupMask) != groupId1 ) {
			log("Bad engineer. Group id 0x%"X INT32 " not derivative of "
			    "group id 0x%"X INT32 " for sectiondb.",
			    groupId2,groupId1);
			return -1;
		}
		...
		if ( (groupId2 & hdb1->m_groupMask) != groupId1 ) {
			log("Bad engineer. Group id 0x%"X INT32 " not derivative of "
			    "group id 0x%"X INT32 " for linkdb.",
			    groupId2,groupId1);
			return -1;
		}
		*/
...
void dumpTitledb (char *coll,int32_t startFileNum,int32_t numFiles,bool includeTree,
 ...
 loop:
	...
	for ( list.resetListPtr() ; ! list.isExhausted() ;
		...
		//if ( rec[0] != 0 ) continue;
		// print it out
		//printf("n1=%08"X INT32 " n0=%016" XINT64 " b=0x%02hhx docId=%012" INT64 " sh=%07"X INT32 " ch=%08"X INT32 " "
...
void dumpTfndb (char *coll,int32_t startFileNum,int32_t numFiles,bool includeTree ,
 ...
 loop:
	...
	for ( list.resetListPtr() ; ! list.isExhausted() ;
		...
		fprintf(stdout,
			"%08"X INT32 " %016" XINT64 " docId=%012" INT64 " "
...
bool addToChecksumdb ( char *coll , TitleRec *tr ) {
	...
	if ( ! s_cdbInit ) {
		...
		for ( int32_t i = 0 ; i < ng ; i++ ){
			...
			// . the %"X INT32 " in "g%" INT32 "" is the group NUM to which the 
	...
	log(LOG_INFO,"mila groupId= %"U INT32 " hostId=%" INT32 "",
	    groupId,dbnum);
	// debug msg
	//log("db: %08"X INT32 " %016" XINT64 " %s",k.n1,k.n0,url->getUrl());
...
bool mergeChecksumFiles ( ) {
	...
	for ( int32_t i = 0 ; i < ng ; i++ ) {
		// . initialize our own internal rdb
		// . the %"X INT32 " in "g%"X INT32 "" is the group id to which the keys
		...
		if ( (k.n0 & 0x01) == 0x01 ) {
			...
			if ( tlist.isExhausted() ) {
				log("db: getlist failed 2 "
				    "i=%" INT32 " n1=%"X INT32 " n0=%" XINT64 ". continuing.",
				    i,k.n1,k.n0);
				continue;
			}
			tlist.resetListPtr();
...
bool genDbs ( char *coll ) {
	...
	for ( int32_t i = 0 ; doChecksumdb && i < ng ; i++ ){
		char name[64];
		// . initialize our own internal rdb
		// . the %"X INT32 " in "g%" INT32 "" is the group NUM to which the keys
		//   in this file belong, the "h%" INT32 "" is the host number that
		//   generated these keys
		sprintf(name,"checksumg%" INT32 "h%" INT32 "db",i,g_hostdb.m_hostId);
		// unlink this file just in case
 ...
 nextRec2:
	...
	if ( tkey < nextKey ) {
		...
		log("db: Encountered corruption in titledb while making "
		    "tfndb. Page = %" INT32 ". "
		    "NextKey.n1=%"U INT32 " %" UINT64 ". "
		    "Key.n1=%"U INT32 " %" UINT64 " "
		    "FirstDocId=%" UINT64 ".",
 ...
 loop:
	...
	for ( list.resetListPtr() ; ! list.isExhausted() ;
		...
		fprintf(stdout,
			...
			"e=0x%08"X INT32 " tfn=%03" INT32 " clean=%" INT32 " half=%" INT32 " %s\n",
...
bool syncIndexdb ( ) {
 ...
 loop:
	...
	nextKey.n0 &= 0xfffffffffffffffeLL;
	// announce startKey
	log("db: next k.n1=%08"X INT32 " n0=%016" XINT64 "",nextKey.n1,nextKey.n0);
	// a niceness of 0 tells it to block until it gets results!!
	Msg5 msg5;
	if ( ! msg5.getList ( RDB_INDEXDB    ,
			      coll1          ,
 ...
 iloop:
	...
	if      ( k1 < k2 ) {
		log("db: sync dir has k.n1=%08"X INT32 " n0=%016" XINT64 "",k1.n1,k1.n0);
		ilist1.skipCurrentRecord();
		count++;
		goto iloop;
	}
	else if ( k2 < k1 ) {
		log("db: orig dir has k.n1=%08"X INT32 " n0=%016" XINT64 "",k2.n1,k2.n0);
...
listLoop:
	...
	if ( count % 10000 == 0 )
		log(LOG_INFO, "db: %" INT32 ") %"X INT32 " %" XINT64 "", count,
...
// 		int32_t endKey = (rand() % (numKeys - startKey)) + startKey;
		...
		"between "
		"%016" XINT64 "%08"X INT32 " and "
		"%016" XINT64 "%08"X INT32 ".  "
		    "took %" INT64 " ms",
		    treelist.getNumRecs(),
		    numPosRecs, 
		    numNegRecs,
 ...
 loop:
	...
	for ( list.resetListPtr() ; ! list.isExhausted() && ! justVerify ;
		...
		if ( termId < 0 )
			...
			printf("k.n1=%016" XINT64 " k.n0=%016" XINT64 " "
			       "date=%010"U INT32 " score=%03" INT32 " docId=%012" INT64 "%s\n" , 
			       KEY1(k,16),KEY0(k),
			       list.getCurrentDate(),
			       (int32_t)list.getScore(k),
			       list.getCurrentDocId() , dd );
	...
	for ( list.resetListPtr() ; ! list.isExhausted() ;
		...
		if ( test != 3 && ! g_statsdb.getResolutionFromKey( k ) ) {
			printf("[Session Header Key] "
			       "k.n1=%08"X INT32 " k.n0=%016" XINT64 " resolution=%03"U INT32 " "
			       "session=%05d timestamp=%010" INT32 " [%s] "
			       "hostId=%05"U INT32 " version=%03"U INT32 " %s\n",
			       k.n1 , k.n0 ,
			...
			printf("k.n1=0x%08"X INT32 " k.n0=0x%016" XINT64 " resolution=%03"U INT32 " "
			       "session=%05d timestamp=%010" INT32 " [%s] "
			       "hostId=%05"U INT32 " version=%03"U INT32 " "
			       "uLen=%010"U INT32 " cLen=%010"U INT32 " uCRC=%016" XINT64 " %s \n",
			       k.n1 , k.n0 ,
			       (uint32_t)g_statsdb.getResolutionFromKey(k),
			       (int16_t)g_statsdb.getSessionFromKey(k) ,
			       (int32_t)dateStamp ,
 ...
 loop:
	...
	for ( list.resetListPtr() ; ! list.isExhausted() ;
		...
		if ( cKeySize == 12 ) {
			...
			sprintf( kBuf, "%08"X INT32 "", ((key_t *)k)->n1);
		}
		else if ( cKeySize == 16 ) {
			// get the language string
		...
		printf("k.n1=%s k.n0=%016" XINT64 " "
		       "docId=%012" INT64 " quality=%d hosthash=0x%04"X INT32 "%s\n",
		       kBuf, ((key_t *)k)->n0,
...
void dumpCachedRecs (char *coll,int32_t startFileNum,int32_t numFiles,bool includeTree,
 ...
 loop:
	...
	for ( list.resetListPtr() ; ! list.isExhausted() ;
		...
		if ( k <= lastKey ) 
			log("key out of order. "
			    "lastKey.n1=%"X INT32 " n0=%" XINT64 " "
			    "currKey.n1=%"X INT32 " n0=%" XINT64 " ",
			    lastKey.n1,lastKey.n0,
			    k.n1,k.n0);
		lastKey = k;
		// print deletes
// 		if ( (k.n0 & 0x01) == 0) {
// 			fprintf(stderr,"n1=%08"X INT32 " n0=%016" XINT64 " docId=%012" INT64 " "
// 			       "hh=%07"X INT32 " ch=%08"X INT32 " (del)\n", 
