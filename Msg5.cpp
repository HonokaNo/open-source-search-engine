...
bool Msg5::getList ( char     rdbId         ,
	...
	*/
	//log("ck.n1=%"U INT32 " ck.n0=%" UINT64 "",m_cacheKey.n1,m_cacheKey.n0);
	//exit(-1);
	...
	*/
	...
	if ( m_maxCacheAge != 0 ) {
		log(LOG_DEBUG,"net: Checking cache for list. "
		    //"startKey.n1=%"U INT32 " %" UINT64 ". cacheKey.n1=%"U INT32 " %" UINT64 ".",
	...
	*/
	...
	// timing debug
	//log("Msg5:getting list startKey.n1=%"U INT32 "",m_startKey.n1);
...
bool Msg5::readList ( ) {
 ...
 readMore:
	...
	QUICKPOLL((m_niceness));
	// debug msg
	//log("msg5 calling msg3 for %" INT32 " bytes (msg5=%"U INT32 ")",
...
bool Msg5::needsRecall ( ) {
	...
	if ( m_round >= 1 && m_totalSize == m_lastTotalSize ) {
		log("msg5: increasing minrecsizes did nothing. assuming done. "
		    ...
		    "cn=%" INT32 " this=0x%"X INT32 " round=%" INT32 ".", 
		    base->m_dbname , 
		    m_newMinRecSizes,
		    m_minRecSizes, 
		    m_list->m_listSize,
...
bool Msg5::gotList ( ) {
	...
	// debug msg
	//log("msg5 got lists from msg3 (msg5=%"U INT32 ")",(int32_t)this);
...
void threadDoneWrapper ( void *state , ThreadEntry *t ) {
	...
	Msg5 *THIS = (Msg5 *)state;
	// debug msg
	//log("msg3 back from merge thread (msg5=%"U INT32 ")",THIS->m_state);
	// . add m_list to our cache if we should
	// . this returns false if blocked, true otherwise
	// . sets g_errno on error
	// . only blocks if calls msg0 to patch a corrupted list
...
void Msg5::repairLists_r ( ) {
	...
	for ( int32_t i = 0 ; i < m_numListPtrs ; i++ ) {
		...
		if ( i < nn && base ) {
			...
			//log("db: "
			//    "startKey.n1=%"X INT32 " n0=%" XINT64 " "
			//    "endKey.n1=%"X INT32 " n0=%" XINT64 "",
...
bool Msg5::gotRemoteList ( ) {
	...
	if ( ! g_errno ) {
		...
		log(LOG_DEBUG,
		    //"net: Received list skey.n1=%08"X INT32 " skey.n0=%016" XINT64 "." ,
		    //  k.n1 , k.n0 );
		    "net: Received list skey=%s." ,
		      KEYSTR(k,m_ks) );
		k = m_list->getEndKey();
		log(LOG_DEBUG,
		    //"net: Received list ekey.n1=%08"X INT32 " ekey.n0=%016" XINT64 "." ,
		    //  k.n1 , k.n0 );
		    "net: Received list ekey=%s",
		      KEYSTR(k,m_ks) );
		if ( ! m_list->isEmpty() ) {
			k = m_list->getLastKey();
			//log(LOG_DEBUG,"net: Received list Lkey.n1=%08"X INT32 " "
