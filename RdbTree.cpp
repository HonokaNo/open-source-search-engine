...
int32_t RdbTree::addNode ( collnum_t collnum , 
	...
	//if ( m_dbname && m_dbname[0]=='t' && dataSize >= 4 )
	//	logf(LOG_DEBUG,
	//	     "adding node #%" INT32 " with data ptr at %"X INT32 " "
...
void RdbTree::gbmprotect ( void *p , int32_t size , int prot ) {
	...
	//	log("db: unprotect: 0x%"X INT32 " size=%" INT32 "",(int32_t)np,nsize);
	//else
	//	log("db: protect: 0x%"X INT32 " size=%" INT32 "",(int32_t)np,nsize);
}
	...
	for ( ; node >= 0 && list->getListSize() < minRecSizes ;
		...
		if ( m_fixedDataSize == 0 ) {
			...
			if ( m_dbname && 
				...
				logf(LOG_DEBUG,
				     "tree: "
				     "getting node #%" INT32 " with data ptr at %"U INT32 " "
				     "and data size of %" INT32 " into a list.",
