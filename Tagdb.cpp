...
bool Tag::printToBuf ( SafeBuf *sb ) {
	...
	//sb->safePrintf("%"U INT32 ",",m_timestamp);
	// print the ip added from
	sb->safePrintf("%s,",iptoa(m_ip));
	// print the tag id
	//sb->safePrintf("%"U INT32 ",\"",(int32_t)m_tagId);
	// key.n1 is hash of the subdomain i think
	//sb->safePrintf("%"U INT32 ",\"",m_key.n1);
	sb->safePrintf("\"");
...
bool Tag::printToBufAsAddRequest ( SafeBuf *sb ) {
	...
	// print the tag id
	//sb->safePrintf("&%s.id=%"U INT32 "",str,(int32_t)m_tagId);
	// the "score"
...
bool Tag::printToBufAsHtml ( SafeBuf *sb , char *prefix ) {
	...
	//sb->safePrintf(" id=%"U INT32 "",(int32_t)m_tagId);
	sb->safePrintf("</td></tr>\n");
	return true;
}
...
bool TagRec::printToBuf (  SafeBuf *sb ) {
	Tag *tag = getFirstTag();
	//sb->safePrintf("k.n1=0x%08"X INT32 " k.n0=0x%016" XINT64 " version=%" INT32 "",
...
int32_t TagRec::setFromBuf ( char *p , char *pend ) {
	...
	// read in the key
	//key_t k;
	//sscanf(p,"k.n1=0x%08"X INT32 " k.n0=0x%016" XINT64 " ",&k.n1,&k.n0);
...
void handleRequest9a ( UdpSlot *slot , int32_t niceness ) {
	...
	if ( slotNum >= 0 ) {
		...
		if ( g_conf.m_logDebugSpider )
			logf(LOG_DEBUG,"tagdb: TAGDB handleRequest9a "
			     "waiting for lock st=0x%"X INT32 " key.n0=%" UINT64 "",(int32_t)st,
	...
	if ( ! st->m_msg5.getList ( RDB_TAGDB      ,
		...
		return;
	// log that for debug
	//log("tagdb: msg5 call did not block. st=%"U INT32 "",(int32_t)st);
...
void gotList ( void *state , RdbList *xxx , Msg5 *yyy ) {
	...
	// note it
	//log("tagdb: in gotlist st=%"U INT32 "",(int32_t)st);
 ...
 loop:
	...
	if ( st && g_conf.m_logDebugSpider ) 
		logf(LOG_DEBUG,"tagdb: calling lock for st=0x%"X INT32 "",(int32_t)st);
